                                        ...
                                        <td class="td-actions">
                                            <h4>
                                                <i class="fa fa-pencil" aria-hidden="true" title="修改" data-toggle="modal" data-target="#modifyModal"
                                                   data-id="{{$board['id']}}"
                                                   data-block="{{$board['block']}}"
                                                   ></i>&nbsp;&nbsp;

                                                <i id="deleteBtn" class="fa fa-trash" aria-hidden="true" title="删除"  data-toggle="modal" data-target="#deleteModal" data-id="{{$board['id']}}"></i>
                                            </h4>
                                        </td>
                                    </tr>
                                @endforeach
                ...
                <div class="modal-body">
                    <form>
                        <div class="form-group">
                            <label for="newBoardName" class="col-form-label">Board</label>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <div class="form-group">
                            <label for="newBoardName" class="col-form-label">Board</label>
                            <input type="text" class="form-control" id="modifyBoardName">
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" onclick="deleteBoard()">Save</button>
                </div>
            </div>
        </div>
    </div>
...
@section('javascript')
    <script>
        ...
        $('#modifyModal').on('show.bs.modal', function (event) {
            ...
            modal.find('#modifyBoardName').val(block)
        })

        function modify(){
            var id = $("#modifyId").val()
            var board = $("#modifyBoardName").val()
            $.ajax({
                url: '/admin/board',
                data: {
                    'boardId': id,
